...
class Fe108(BaseVideoDataset):
    ...
    def _get_event_sequence_path(self, seq_id):        ## get evemts' frames
        return os.path.join('/wangx/DATA/Dataset/fe108/new_voxel/train/', self.sequence_list[seq_id], self.sequence_list[seq_id] + "_5binvoxel")

    def _get_event_frame(self, seq_path, frame_id):
        frame_event_list = []
        for f_id in frame_id:
            event_frame_file = os.path.join(seq_path, 'frame{:04}.npz'.format(f_id))
            if os.path.getsize(event_frame_file) == 0:
                event_features = np.zeros(260, 346, 5)
                # need_data = [np.zeros([4096, 3]), np.zeros([4096, 16])]
            else:
                mat_data = np.load(event_frame_file)
                event_features = mat_data['features'].transpose(1, 2, 0)
