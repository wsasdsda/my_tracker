    ...
    def __call__(self, data: TensorDict):
        ...
        for s,r in zip(['template', 'search'],['template_event', 'search_event']) :
            ...
            crops_r,boxes_r, att_mask_r, mask_crops_r, crop_coor_r = prutils.jittered_center_crop(data[r + '_images'],                                                                                     
                                                                                                ...
                                                                                                self.output_sz[s],
                                                                                                masks=data[s + '_masks'])
            crops_v,boxes_v, att_mask_v, mask_crops_v, crop_coor_v = prutils.jittered_center_crop(data[r + '_voxel'],                                                                                     
                                                                                                jittered_anno,
                                                                                                data[s + '_anno'],
                                                                                                self.search_area_factor[s],
                                                                                                self.output_sz[s],
            ...
            data[r + '_voxel'] = [torch.from_numpy(crops_v[i]).permute(2,0,1).contiguous() for i in range(len(crops_v))]      
